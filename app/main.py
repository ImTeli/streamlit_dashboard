...
with aba1:
    st.dataframe(tayframe.df)

with aba2:
    coluna1, coluna2 = st.columns(2)
    with coluna1:
        st.metric("Receita Total", tayframe.format_number(df["Preço"].sum(), prefix="R$"))
        st.plotly_chart(tayframe.map_estado_chart(), use_container_width=True)
        st.plotly_chart(tayframe.rec_estado_chart(), use_container_width=True)
    with coluna2:
        st.metric("Quantidade de vendas", (df.shape[0]))
        st.plotly_chart(tayframe.rec_mensal_chart(), use_container_width=True)
        st.plotly_chart(tayframe.rec_categoria_chart(), use_container_width=True)

with aba3:
    coluna1, coluna2 = st.columns(2)
    with coluna1:
        st.plotly_chart(tayframe.rec_vendedores_chart())
    with coluna2:
        st.plotly_chart(tayframe.vendas_vendedores_chart())
